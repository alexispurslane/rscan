...
(define (virustotal-test names sha)
  ...
  (define-values (status-code header inport)
    (http-sendrecv "www.virustotal.com" "/vtapi/v2/file/report" #:method "POST"
     #:data (alist->form-urlencoded
             (list (cons 'apikey API-KEY) (cons 'resource sha)))))
...
(define (draw-box inner-text)
  (define (remove-colors s)
    (~> s (string-replace "\033[91m" "") (string-replace "\033[92m" "")
        (string-replace "\033[1m" "") (string-replace "\033[0m" "")))
  ...
  (define wall+text (map (lambda (x)
                           (let* ([diff (- wall-len (string-length (remove-colors x)) 1)]
...
(define (gen-positives p t) (format "Positives: ~a% (~a/~a)" (ceiling (* 100 (/ p t))) p t))
